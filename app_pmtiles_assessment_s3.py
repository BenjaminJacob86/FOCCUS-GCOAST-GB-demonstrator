...
def _indicator_nc_s3_uri(
    run_date: dt.date,
    nc_file: str,
    *,
    scenario_subfolder: str | None = None,
) -> str:
    return indicator_s3_uri(
        run_date, nc_file, scenario_subfolder=scenario_subfolder
    )
...
def run_dashboard(*, configure_page: bool = True) -> None:
    cfg = get_config()
    indicator_layers = cfg.indicator_layers
    logo_path = cfg.logo_primary if cfg.logo_primary.is_file() else cfg.logo_fallback

    if configure_page:
        st.set_page_config(
            page_title=cfg.dashboard_page_title,
            layout="wide",
    ...
    with hdr_text:
        st.markdown(f"**{cfg.header_title}**")
    ...
    scenario_labels = {s.key: s.label for s in scenarios}
    with st.container(border=True):
        if scenarios:
            tb_run, tb_scen, tb_ind, tb_crit = st.columns(
                [1, 1, 1, 1], vertical_alignment="bottom"
            )
        else:
            tb_run, tb_ind, tb_crit = st.columns([1, 1, 1], vertical_alignment="bottom")
        with tb_run:
            run_date = st.date_input("Run date", value=cfg.default_run_date)
        ...
        if scenarios:
            ...
            default_scenario_key = cfg.default_scenario_key or scenario_keys[0]
            default_scenario_idx = (
                scenario_keys.index(default_scenario_key)
                if default_scenario_key in scenario_keys
                else 0
            )
            with tb_scen:
                scenario_key = st.selectbox(
                    ...
                    help=(
                        "Vegetation-free, with-vegetation, or precomputed difference "
                        "layers (separate S3 subfolders per scenario)."
                    ),
                )
            scenario = next(s for s in scenarios if s.key == scenario_key)
            scenario_subfolder = scenario.subfolder or None

        with tb_ind:
            indicator_label = st.selectbox("Indicator", options=indicator_names, index=0)
        layer_cfg = indicator_layers[indicator_label]

        pmtiles_uri = indicator_s3_uri(
            run_date, layer_cfg["file"], scenario_subfolder=scenario_subfolder
        )
        nc_uri = _indicator_nc_s3_uri(
            run_date, layer_cfg["nc_file"], scenario_subfolder=scenario_subfolder
        )
        ...
        with tb_crit:
            if is_erosion_r1:
                ...
                critical_level = None
                crit_default = float(layer_cfg.get("critical_default", 0.0))
                if scenario is not None and scenario.is_difference:
                    crit_default = float(layer_cfg.get("diff_critical_default", 0.0))
        ...
        if Debug:
            scen_part = f"{scenario_subfolder}/" if scenario_subfolder else ""
            st.caption(f"S3 folder: `{cfg.s3_prefix}/{run_folder}/{scen_part}`")
    ...
    try:
        ...
        index_uri = indicator_s3_uri(
            run_date, MESH_INDEX_FILENAME, scenario_subfolder=scenario_subfolder
        )
    ...
    if scenario is not None and scenario.is_difference:
        default_cmap = str(layer_cfg.get("diff_cmap", "coolwarm"))
    ...
    tile_options = {
        ...
        # CartoDB tiles now require an API key — keep options for future use:
        ...
        with st.container(height=THR_HEIGHT + 75, border=True):
            ...
            if not has_data or thr_rgba is None or thr_bounds is None:
                ...
                try:
                    ...
                    tm = folium.Map(
                        location=tcenter, zoom_start=8, tiles="OpenStreetMap", control_scale=True
                    )
