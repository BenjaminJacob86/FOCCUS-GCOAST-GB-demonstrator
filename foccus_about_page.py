...
def render_about_page() -> None:
    about_md = get_config().about_markdown
    if not about_md.is_file():
        st.error(f"Documentation file not found: `{about_md.name}`")
        return

    md_body = _normalize_legacy_iframes(about_md.read_text(encoding="utf-8"))
