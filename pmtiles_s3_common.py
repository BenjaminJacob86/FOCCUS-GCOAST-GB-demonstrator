...
def public_s3_url(bucket: str, key: str) -> str:
    ...
    """
    return f"{get_config().endpoint_url.rstrip('/')}/{bucket}/{key.lstrip('/')}"
...
def effective_map_bounds(
    ...
    if layer_cfg.get("use_domain_bounds") or span_lon < MIN_BOUNDS_SPAN_DEG or span_lat < MIN_BOUNDS_SPAN_DEG:
        ds, dw, dn, de = get_config().domain_bounds
        return (ds, dw, dn, de)
    return bounds


def indicator_s3_uri(
    run_date: dt.date,
    filename: str,
    *,
    ...
    parts = [cfg.s3_prefix, folder]
    if scenario_subfolder:
        parts.append(scenario_subfolder.strip("/"))
    parts.append(filename)
    return f"s3://{cfg.s3_bucket}/{'/'.join(parts)}"
